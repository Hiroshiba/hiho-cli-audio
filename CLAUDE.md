...
- **解決できないエラーは全てユーザーにElectronダイアログで通知する**
  - メインプロセス・レンダラープロセス問わず統一されたエラー表示
  - `ErrorDialogService`を使用してElectronの`dialog.showMessageBox`で表示
  - エラー文面をクリップボードにコピーするボタンを提供
  - IPC通信でレンダラーのエラーもメインプロセスのダイアログシステムに集約

#### エラーハンドリング方針

- **想定外の挙動では例外を投げる**
- **フォールバック処理は極力避ける**
- **解決できないエラーはErrorDialogServiceでダイアログ表示**
  - ネットワークエラー、API認証エラー、システムエラー等
  - console.errorと併用してログも残す
- **解決可能なエラーは適切に処理**
  - バリデーションエラー、一時的な状態エラー等
  - ダイアログ表示は不要
- **メインプロセスのエラーでもアプリケーションを継続**
